import { createConfig, validateConfig } from '../'

describe('Mutations Config', () => {
  describe('validateConfig', () => {
    it(`Generator isn't a function`, () => {
      try {
        validateConfig({ value: true }, { value: true })
    ...
    it(`Nested generator isn't a function`, () => {
      try {
        validateConfig({ something: {} }, { something: {} })
        throw Error('This should never happen...')
      } catch (e) {
        expect(e.message).toBe(
          'Config generators must be a function, or an object that contains functions.',
        )
      }
    })
    ...
    it(`Argument doesn't exist`, () => {
      try {
        validateConfig({}, { value: (arg: string) => arg })
        throw Error('This should never happen...')
      } catch (e) {
        expect(e.message).toBe(
