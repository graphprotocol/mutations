import { v4 } from 'uuid'
import { CoreState, CoreEvents, TransactionCreatedEvent } from '../core'
import {
  Event,
  EventPayload,
  MutationState,
...
describe('Core Mutation State', () => {
  ...
  beforeEach(() => {
    ...
    observer = new MutationStateSubject<CoreState, CoreEvents>(
      {} as MutationState<CoreState>,
    )

    state = new StateUpdater<CoreState, CoreEvents>(uuid, undefined, observer)
  ...
  it('Dispatches state updates in correct order', async () => {
    ...
    expect(currentState.events[0].payload).toEqual({ value: .05 })
  ...
  it('Fails if PROGRESS_UPDATE event receives number lower than 0 or higher than 100 or non integer', async () => {
    expect(state.dispatch('PROGRESS_UPDATE', { value: 1.05 } as any)).rejects.toThrow()
    expect(state.dispatch('PROGRESS_UPDATE', { value: -.05 } as any)).rejects.toThrow()
  })
})
...
describe('Extended Mutation State', () => {
  ...
  beforeEach(() => {
    uuid = v4()
    observer = new MutationStateSubject<State, EventMap>(
      {} as MutationState<State, EventMap>,
    )
    state = new StateUpdater<State, EventMap>(uuid, stateBuilder, observer)
  ...
  it('Does not execute catch-all reducer if custom event reducer is defined', async () => {
  ...
  it('Executes both core reducer and catch-all reducer if a core event is supported in it', async () => {
    ...
    expect(state.current.progress).toEqual(.5)
